...
int main(int argc, char **argv)
{
    ...
    for (;;)
    {
        retcode = devils_host_service(server, /* host to service */
        ...
        if (retcode > 0)
        {
            ...
            switch (event.type)
            {
            case DEVILS_EVENT_TYPE_CONNECT:
                /* Create a client */
                client = (Client *)calloc(1, sizeof(Client));
                client->ip[0] = '?';
                devils_address_get_host_ip(&(event.peer->address), client->ip, sizeof(client->ip));
            ...
            case DEVILS_EVENT_TYPE_RECEIVE:
                ...
                /* Send it back. */
                devils_peer_send(event.peer, event.channelID, packet);
                break;
